...
def build_dashboard_config(entry: ConfigEntry) -> dict:
    ...
    # Tasten haben keine eigene HA-Entity mehr (wir hören direkt auf das
    # MQTT-Event des physischen Buttons). Darum "type: button" statt "tile" -
    # das funktioniert ohne gebundene Entität und ruft per Klick dieselbe
    # Aktion auf, die auch beim physischen Drücken ausgeführt wird.
    button_cards = []
    for b in buttons:
        position = b.get(CONF_POSITION)
        if position is None:
            continue
        action = b.get("action") or {}
        service = action.get("action") or action.get("service")
        subtitle = f"Pos {position} · Seite {b.get(CONF_PAGE, 1)} · {b.get(CONF_EVENT_TYPE, 'click')}"
        card = {
            "type": "button",
            "name": f"{b.get('label') or f'Taste {position}'} ({subtitle})",
    ...
    display_entities = [
        {"entity": r["entity_id"], "name": r.get("label") or r["entity_id"]}
